...
Promise.prototype.then = function(f, r) {
	...
	return new Promise(function (resolve, reject) {
		prev._resolver(function (x) {
			joinAssimilate(prev, x, onFulfill, onReject);
		}, onReject);
...
// Fully traverse x to its ultimate fulfillment value
function joinAssimilate(self, x, fulfill, reject) {
	if(x === self) {
		...
		x.done(function(x) {
			joinAssimilate(self, x, fulfill, reject);
		}, reject);
		...
		if(typeof untrustedThen === 'function') {
			call(untrustedThen, x, function(value) {
				joinAssimilate(self, value, fulfill, reject);
			}, reject);
		} else {
			fulfill(x);
		}
...
function runQueue(promise, start, value) {
...
function cast(x) {
	return x instanceof Promise ? x : of(x).then();
}
